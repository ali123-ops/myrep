...
if($_SESSION['loggato']){	
	...
	switch($sc){
		...
		case "listaProprietari":{
			...
			$rs = $db->query($sql);
			showResultTable($rs, "Lista dei Proprietari", "proprietari.php", "mostraDettagli", "proprietari.php", "formmodificaprop", "proprietari.php", "eliminaprop");
			$db->close();
			break;
		}
		case "mostraDettagli":{
			...
			//Form per togliere l'immobile associato
			if($rs->num_rows > 0){
				echo("
					<form action=\"proprietari.php\" method=\"get\">
					<div class=\"row\">
						<div class=\"col\">
							<div class=\"mb-3\">
								<label for=\"idImmobile\" class=\"form-label\">ID del Immobile:</label>
								<select id=\"idImmobile\" name=\"idImmobile\">");
								$rs = $db->query($sql);
								$r= $rs->fetch_assoc();							
								while($r){
					...
					<div class=\"col\">
						...
						<input type=\"submit\" class=\"btn btn-primary\" value=\"Cancella Immobile\">
					</div>
					</div>
					</form>");        
				echo("<br />");		
		...
		// Case per togliere immobile associato
		case "deletePossiede":{
			$idP = $_REQUEST['idProprietario'];
			$idI = $_REQUEST['idImmobile'];
			$db = new mysqli(DBHOST, DBUSER, DBPASSWORD, DBNAME);
			$sql = "DELETE FROM p73e6_possiede WHERE idProprietario='$idP' AND idImmobile='$idI'";
			if($db->query($sql)){
				echo("<div class=\"alert alert-success\">Cancellazione acquisto completato.</div>");
			}
			else{
				echo("<div class=\"alert alert-danger\">Cancellazione ccquisto non completato.</div>");
			}
		...
		// case per modificare proprietario
		case "formmodificaprop":{
			$idR = $_REQUEST['idRecord'];
				...
				<form action=\"proprietari.php\" method=\"get\">
					<div class=\"container\">
						<div class=\"row\">
							<div class=\"col-6\">
								<div class=\"mb-3\">
								   <label for=\"moCognome\" class=\"form-label\">Cognome:</label>
								   <input class=\"form-control\" type=\"text\" name=\"cognome\" id=\"moCognome\" placeholder=\"\" aria-label=\"default input example\" value=\"".$record['cognome']."\">
								</div>
							</div>
							<div class=\"col-6\">
								<div class=\"mb-3\">
									<label for=\"moNome\" class=\"form-label\">Nome:</label>
									<input class=\"form-control\" type=\"text\" name=\"nome\" id=\"moNome\" placeholder=\"\" aria-label=\"default input example\" value=\"".$record['nome']."\">
								</div>
							</div>
						</div>
						...
						<div class=\"row\">
							<div class=\"col-6\">
								<div class=\"mb-3\">
									<label for=\"moVia\" class=\"form-label\">Via:</label>
									<input class=\"form-control\" type=\"text\" name=\"via\" id=\"moVia\" placeholder=\"\" aria-label=\"default input example\" value=\"".$record['via']."\">
								</div>
							</div>
							<div class=\"col-1\">
								<div class=\"mb-3\">
									<label for=\"moCivico\" class=\"form-label\">Civico:</label>
									<input class=\"form-control\" type=\"text\" name=\"civico\" id=\"moCivico\" placeholder=\"\" aria-label=\"default input example\" value=\"".$record['civico']."\">
								</div>
							</div>
							<div class=\"col-5\">
								<div class=\"mb-3\">
									<label for=\"moCitta\" class=\"form-label\">Città:</label>
									<input class=\"form-control\" type=\"text\" name=\"citta\" id=\"moCitta\" placeholder=\"\" aria-label=\"default input example\" value=\"".$record['citta']."\">
								</div>
							</div>
						</div>
						...
						<div class=\"row\">
							<div class=\"col-6\">
								<div class=\"mb-3\">
									<label for=\"moTelefono\" class=\"form-label\">Telefono:</label>
									<input class=\"form-control\" type=\"text\" name=\"telefono\" id=\"moTelefono\" placeholder=\"\" aria-label=\"default input example\" value=\"".$record['telefono']."\">
								</div>
							</div>
							<div class=\"col-6\">
								<div class=\"mb-3\">
									<label for=\"moMail\" class=\"form-label\">Mail:</label>
									<input class=\"form-control\" type=\"text\" name=\"mail\" id=\"moMail\" placeholder=\"\" aria-label=\"default input example\" value=\"".$record['mail']."\">
								</div>
							</div>
						</div>
						...
						<div class=\"row\">
							<div class=\"col\">
								...
								<input type=\"hidden\" name=\"sc\" value=\"modificaprop\">
								<button type=\"submit\" class=\"btn btn-primary\">Inserisci</button>
							</div>
						</div>
					</div>
		...
		case "modificaprop":{
			...
			$db = new mysqli(DBHOST, DBUSER, DBPASSWORD, DBNAME);
			$sql = "UPDATE p73e6_proprietario SET cognome='$cognome', nome='$nome', via='$via', civico='$civico', citta='$citta', telefono='$telefono', mail='$mail' WHERE id=$idR";
			if($db->query($sql))
				echo("<div class=\"alert alert-success\">Modificato</div>");
			else{
				echo("<div class=\"alert alert-danger\">Errore nella modifica</div>");
			}
			break;
		}
		// case per eliminare proprietario
		case "eliminaprop":{
			$idR = $_REQUEST['idRecord'];
		
			$db = new mysqli(DBHOST, DBUSER, DBPASSWORD, DBNAME);
			$sql = "DELETE FROM p73e6_proprietario WHERE id=$idR";
			if($db->query($sql))
