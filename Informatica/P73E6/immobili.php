...
if($_SESSION['loggato']){
	...
	switch($sc){
		...
		case "listaImmobili":{
			...
			$rs = $db->query($sql);
			showResultTable($rs, "Lista degli Immobili","","", "immobili.php", "formmodificaImmobile", "immobili.php", "eliminaImmobile");
			$db->close();
			break;
		}
		case "formmodificaImmobile":{
			$idR = $_REQUEST['idRecord'];
			
			$db = new mysqli(DBHOST, DBUSER, DBPASSWORD, DBNAME);
			$sql = "SELECT * FROM p73e6_immobile WHERE  id=$idR";
			$rs = $db->query($sql);
			$record = $rs->fetch_assoc();
			
			echo("
				<div class=\"alert alert-success\">Form di modifica Immobile</div>
				<form action=\"immobili.php\" method=\"get\">
					<div class=\"container\">
						<div class=\"row\">
							<div class=\"col-6\">
					...
					<div class=\"row\">
						<div class=\"col\">
							<div class=\"mb-3\">
								<label for=\"moMetratura\" class=\"form-label\">Metratura:</label>
								<input class=\"form-control\" type=\"text\" name=\"metratura\" id=\"moMetratura\" placeholder=\"\" aria-label=\"default input example\" value=\"".$record['metratura']."\">
							</div>
						</div>                     
						<div class=\"col\">
							<div class=\"mb-3\">
								<label for=\"moPiano\" class=\"form-label\">Piano:</label>
								<input class=\"form-control\" type=\"text\" name=\"piano\" id=\"moPiano\" placeholder=\"\" aria-label=\"default input example\" value=\"".$record['piano']."\">
							</div>
						</div>
						<div class=\"col\">
							<div class=\"mb-3\">
								<label for=\"moLocali\" class=\"form-label\">Locali:</label>
								<input class=\"form-control\" type=\"text\" name=\"locali\" id=\"moLocali\" placeholder=\"\" aria-label=\"default input example\" value=\"".$record['locali']."\">
							</div>
						</div>
					</div>
					...
					<div class=\"row\">
						<div class=\"col\">
							<div class=\"mb-3\">
								<label for=\"inZona\" class=\"form-label\">Zona Cittadina:</label>
								<select class=\"form-select\" name=\"zona\" id=\"moZona\" aria-label=\"Default select example\">");
								$db = new mysqli(DBHOST, DBUSER, DBPASSWORD, DBNAME);
								$sql = "SELECT * FROM p73e6_zona";
								$rs = $db->query($sql);
								while($record = $rs->fetch_assoc())
						...
						<div class=\"col\">
							<div class=\"mb-3\">
								<label for=\"moTipologia\" class=\"form-label\">Tipologia Immobile:</label>
								<select class=\"form-select\" name=\"tipologia\" id=\"moTipologia\" aria-label=\"Default select example\">");
								$db = new mysqli(DBHOST, DBUSER, DBPASSWORD, DBNAME);
								$sql = "SELECT * FROM p73e6_tipologia";
								$rs = $db->query($sql);
								while($record = $rs->fetch_assoc())
					...
					<div class=\"row\">
						<div class=\"col\">
							...
							<input type=\"hidden\" name=\"sc\" value=\"modificaImmobile\">
							<button type=\"submit\" class=\"btn btn-primary\">Inserisci</button>
						</div>
					</div>
				</div>
		...
		case "modificaImmobile":{
			...
			$db = new mysqli(DBHOST, DBUSER, DBPASSWORD, DBNAME);
			$sql = "UPDATE p73e6_immobile
					SET nome='$nome', via='$via', civico='$civico', metratura='$metratura', piano= '$piano', locali= '$locali', idZona= '$zona', idTipologia='$tipologia'
					WHERE id=$idR";
			if($db->query($sql)){
			...
			echo("<a href=\"immobili.php?sc=listaImmobili\"><input type=\"button\" class=\"btn btn-primary\"value=\"Back\"></a>");
			$db->close();
			break;
		}
		case "eliminaImmobile":{
			...
			$db = new mysqli(DBHOST, DBUSER, DBPASSWORD, DBNAME);
			$sql = "DELETE FROM p73e6_immobile WHERE id=$idR";
			if($db->query($sql))
